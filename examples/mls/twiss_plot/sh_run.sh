...
echo $energy
	...
	echo -e "\n##### Starting elegant #####\n"	
#	elegant ele_twissOutput.ele -macro=rootname=$outfile,latticename=$lattice
	elegant ele_twissOutput.ele -macro=rootname=$outfile,energy_mev=$energy,latticename=$lattice > output_ele.log
	
	echo -e "\n##### TwissParameters using SDDS #####\n"
	sh sh_GlobalTwiss.sh $outfile.twi $outfile.rf > $outfile.twissPara.txt
	
	echo -e "\n##### Plot twiss with python #####\n"
	python plotTwiss.py $outfile.twi
	
	evince $outfile.twissPlot.pdf &
