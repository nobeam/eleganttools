...
class SDDS:
    ...
    def load(self, input):
        ...
        try:
            ...
            for i in range(numberOfParameters):
                self.parameterDefinition[i] = sddsdata.GetParameterDefinition(
            ...
            for i in range(numberOfColumns):
                self.columnDefinition[i] = sddsdata.GetColumnDefinition(
                    self.index, self.columnName[i]
                )
    ...
    def save(self, output):
        ...
        try:
            ...
            for page in range(pages):
                ...
                for i in range(numberOfParameters):
                    if (
                        sddsdata.SetParameter(
                            self.index, i, self.parameterData[i][page]
                        )
                        != 1
                    ):
                        sddsdata.PrintErrors(self.SDDS_EXIT_PrintErrors)
    ...
    def defineParameter(
        self, name, symbol, units, description, formatString, type, fixedValue
    ):
    ...
    def defineColumn(
        self, name, symbol, units, description, formatString, type, fieldLength
    ):
