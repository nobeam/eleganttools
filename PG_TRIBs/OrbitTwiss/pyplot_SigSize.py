...
# Length of ring
C = np.max(np.array(twiCore.s,dtype=np.float64))
print '\n twi3IB...', len(twiCore), len(twiCore.s), twiCore.keys(), C
...
etaxpCore = etaxpCore.astype(np.float)	
...
etaxpCoreturn1 = etaxpCore[0*npoints : 1*npoints+1] 
etaxpCoreturn2 = etaxpCore[1*npoints : 2*npoints+1] 
etaxpCoreturn3 = etaxpCore[2*npoints : 3*npoints+1]

emittancex = 7.7e-9 #emittancex in m rad 
delta = 7.0e-4

# sigxCoreturn1 = np.sqrt(emittancex * np.array(bxCoreturn1,dtype=np.float64))
# sigxCoreturn1 = np.sqrt(m.pow(delta,2) * np.power(etaxCoreturn1, 2))
sigxCoreturn1 = 1.0e3 * np.sqrt(emittancex * np.array(bxCoreturn1,dtype=np.float64) + m.pow(delta,2) * np.power(etaxCoreturn1, 2))

# divxCoreturn1 = np.sqrt(emittancex / np.array(bxCoreturn1,dtype=np.float64))
# divxCoreturn1 = np.sqrt(m.pow(delta,2) * np.power(etaxpCoreturn1, 2))
divxCoreturn1 = 1.0e3 * np.sqrt(emittancex / np.array(bxCoreturn1,dtype=np.float64) + m.pow(delta,2) * np.power(etaxpCoreturn1, 2))
print sigxCoreturn1

# ## Create figure in golden ratio (A paper sizes)
# fig, (axOrbit, axAngle) = plt.subplots(2, 1, sharex=True, figsize=(15,8))
figsizeinch = 14
fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(figsizeinch,figsizeinch * 0.2**.5))
# ax1.plot([0.0, C/3.0], [0.0, 0.0], 'k-', lw=1)
ax1.plot(sturn1, sigxCoreturn1, 'k-', lw=2, label='Core')
...
# ax2.plot(sturn1, etaxpCoreturn1, 'k-', lw=1, label='Core')
ax2.plot(sturn1, divxCoreturn1, 'k-', lw=2, label='Core')
# ax2.plot(sturn1, etaxCoreturn1, 'k-', lw=4, label='Core')
...
ax2.set_ylabel(r'$\sigma^{\prime}_{x}$ / m rad', size=14)
...
tp.paintlattice(twiCore,0,C/3.0, ec=False, labels=False, fscale=20)
tp.plotstraightnames(14,8,0.82)
